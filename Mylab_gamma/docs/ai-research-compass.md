...
- `data/ai-compass/lab_research_tags.json`
  - 研究室ごとの研究タグ、重み、根拠を管理します。初期版は `labs.json` の構造化タグをもとに生成しています。
...
未登録語を確認し、必要なものだけ `data/ai-compass/general_dictionary.json` または `data/ai-compass/research_bridge.json` に追記してください。

研究室の出方が弱い場合は、`data/ai-compass/lab_research_tags.json` にその研究室の語や研究タグを追記してください。

判断に迷う語は、ChatGPT等に次のように依頼します。

```text
My Labタグ体系に合わせて「クラゲ」を分類してください。
既存の general_dictionary.json / research_bridge.json / lab_research_tags.json に寄せて、追記候補をJSON形式で出してください。
```
...
未登録語が多い領域は、`data/ai-compass/general_dictionary.json` に `keyword,aliases,concepts,weight` を追記して再検証します。

一般概念から研究タグへの接続が弱い場合は、`data/ai-compass/research_bridge.json` を調整します。

特定の研究室が出にくい場合は、`data/ai-compass/lab_research_tags.json` に `research_tags` や `keywords` を追記して再検証します。
