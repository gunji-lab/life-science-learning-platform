...
normalize text, extract dictionary tags, score labs by matching tags, and
summarize gaps that should guide dictionary updates.
"""
...
def score_labs(tags: Counter[str], labs: list[dict[str, object]]) -> list[tuple[int, dict[str, object], list[str]]]:
    scored = []
    for lab in labs:
        terms = lab_terms(lab)
        matched = [tag for tag in tags if tag in terms]
        score = sum(tags[tag] for tag in matched)
        if score:
